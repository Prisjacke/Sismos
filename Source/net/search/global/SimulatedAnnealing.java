...
public class SimulatedAnnealing extends GlobalScoreSearchAlgorithm
    ...
    static final long serialVersionUID = 0xb3e9757dde97e11cL;
    double m_fTStart;
    double m_fDelta;
    int m_nRuns;
    ...
    public void search(BayesNet bayesnet, Instances instances) throws Exception {
        ...
        for (int i = 0; i < m_nRuns; i++) {
            ...
            do {
                ...
                int j = Math.abs(m_random.nextInt()) % instances.numAttributes();
                int k;
                for (k = Math.abs(m_random.nextInt()) % instances.numAttributes(); j == k; k = Math.abs(m_random.nextInt()) % instances.numAttributes()) { }
                if (isArc(bayesnet, k, j)) {
                    flag = true;
                    bayesnet.getParentSet(k).deleteParent(j, instances);
                    double d6 = calcScore(bayesnet);
                    double d4 = d6 - d;
                ...
                if (addArcMakesSense(bayesnet, instances, k, j)) {
                    flag = true;
                    double d7 = calcScoreWithExtraParent(k, j);
                    double d5 = d7 - d;
