...
public class GeneticSearch extends LocalScoreSearchAlgorithm {
    class BayesNetRepresentation
        ...
        void calcScore() {
            ...
            for (int j = 0; j < m_nNodes; j++) {
                ...
                for (int l = 0; l < m_nNodes; l++) {
                    if (m_bits[l + j * m_nNodes]) {
                        parentset1.addParent(l, m_BayesNet.m_Instances);
                    }
                }
    ...
    static final long serialVersionUID = 0x9e574d80642a5e53L;
    int m_nRuns;
    ...
    protected void search(BayesNet bayesnet, Instances instances) throws Exception {
        ...
        for (int j = 0; j < getPopulationSize(); j++) {
            abayesnetrepresentation[j] = new BayesNetRepresentation(instances.numAttributes());
            abayesnetrepresentation[j].randomInit();
            if (abayesnetrepresentation[j].getScore() > d) {
                copyParentSets(bayesnet1, bayesnet);
                d = abayesnetrepresentation[j].getScore();
            }
        }

        for (int k = 0; k < m_nRuns; k++) {
            BayesNetRepresentation abayesnetrepresentation1[] = new BayesNetRepresentation[getDescendantPopulationSize()];
            for (int l = 0; l < getDescendantPopulationSize(); l++) {
                abayesnetrepresentation1[l] = abayesnetrepresentation[m_random.nextInt(getPopulationSize())].copy();
                if (getUseMutation()) {
                    if (getUseCrossOver() && m_random.nextBoolean()) {
                        abayesnetrepresentation1[l].crossOver(abayesnetrepresentation[m_random.nextInt(getPopulationSize())]);
                    } else {
                        abayesnetrepresentation1[l].mutate();
                    }
                } else {
                    abayesnetrepresentation1[l].crossOver(abayesnetrepresentation[m_random.nextInt(getPopulationSize())]);
                }
                if (abayesnetrepresentation1[l].getScore() > d) {
                    copyParentSets(bayesnet1, bayesnet);
                    d = abayesnetrepresentation1[l].getScore();
                }
            }

            boolean aflag[] = new boolean[getDescendantPopulationSize()];
            for (int i1 = 0; i1 < getPopulationSize(); i1++) {
                int j1 = 0;
                if (m_bUseTournamentSelection) {
                    ...
                    int k1;
                    for (k1 = m_random.nextInt(getDescendantPopulationSize()); aflag[k1]; k1 = (k1 + 1) % getDescendantPopulationSize()) { }
                    if (abayesnetrepresentation1[k1].getScore() > abayesnetrepresentation1[j1].getScore()) {
                        j1 = k1;
                    }
                } else {
                    for (; aflag[j1]; j1++) { }
                    double d1 = abayesnetrepresentation1[j1].getScore();
                    for (int l1 = 0; l1 < getDescendantPopulationSize(); l1++) {
                        if (!aflag[l1] && abayesnetrepresentation1[l1].getScore() > d1) {
                            d1 = abayesnetrepresentation1[l1].getScore();
                            j1 = l1;
                        }
                    }

                }
                abayesnetrepresentation[i1] = abayesnetrepresentation1[j1];
                aflag[j1] = true;
            }
